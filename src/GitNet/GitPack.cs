        ...
        private byte[] ApplyPatch(byte[] delta, byte[] origin)
        {
            ...
            while (deltaStream.Position < deltaStream.Length)
            {
                ...
                if ((opCode & 128) != 0)
                {
                    int offset = 0;
                    if ((opCode & 0x01) != 0)
                        offset |= deltaStream.ReadByte();
                    if ((opCode & 0x02) != 0)
                        offset |= deltaStream.ReadByte() << 8;
                    if ((opCode & 0x04) != 0)
                        offset |= deltaStream.ReadByte() << 16;
                    if ((opCode & 0x08) != 0)
                        offset |= deltaStream.ReadByte() << 24;

                    int length = 0;
                    if ((opCode & 0x10) != 0)
                        length |= deltaStream.ReadByte();
                    if ((opCode & 0x20) != 0)
                        length |= deltaStream.ReadByte() << 8;
                    if ((opCode & 0x40) != 0)
                        length |= deltaStream.ReadByte() << 16;
